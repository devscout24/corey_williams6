                ...
                <div class="px-3 py-2 border-bottom d-flex align-items-center gap-2 bg-light">
                    <form method="get" action="{{ route('item-kits.index') }}" class="d-flex gap-2 flex-grow-1 m-0">
                        <input type="text" name="search" class="form-control form-control-sm"
                            placeholder="Search item kits..." value="{{ request('search') }}" style="max-width: 300px;" />
                        ...
                        <button type="submit" class="btn btn-sm btn-outline-primary"><i class="bi bi-search"></i>
                            Search</button>
                                ...
                                <tr>
                                    <td class="ps-4">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                <div class="fw-bold"><a href="{{ route('item-kits.edit', $kit->id) }}"
                                                        class="text-primary text-decoration-underline">{{ $kit->name }}</a>
                                                </div>
                                    ...
                                    <td>
                                        <span
                                            class="badge bg-light text-dark border">{{ $kit->category->name ?? 'None' }}</span>
                                    </td>
                                    <td>{{ $kit->supplier->company_name ?? '—' }}</td>
                                    <td class=""><span
                                            class="fw-medium">{{ $kit->cost_price !== null ? $baseCurrencySymbol . number_format((float) $kit->cost_price, $baseDecimals) : '—' }}</span>
                                    </td>
                                    ...
                                    <td class="text-end pe-4">
                                        <div class="btn-group">
                                            <a class="btn btn-sm btn-outline-secondary"
                                                href="{{ route('item-kits.edit', $kit->id) }}" data-bs-toggle="tooltip"
                                                title="Edit">
                                                <i class="bi bi-pencil"></i>
                                            </a>
                                            <button type="button" class="btn btn-sm btn-outline-danger delete-btn"
                                                data-id="{{ $kit->id }}" data-bs-toggle="tooltip" title="Archive">

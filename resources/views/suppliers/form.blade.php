...
<div class="container-fluid">
    <form method="post" action="{{ $supplier ? route('suppliers.update', $supplier->person_id) : route('suppliers.store') }}" class="needs-validation" enctype="multipart/form-data">
        @csrf
        @if($supplier)
            @method('put')
        @endif
                    ...
                    <div class="tab-pane fade show active" id="basic" role="tabpanel" aria-labelledby="basic-tab">
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="first_name" name="first_name" value="{{ old('first_name', $person?->first_name) }}" />
                            </div>
                            <div class="col-md-6">
                                <label class="form-label" for="last_name">Last Name</label>
                                <input type="text" class="form-control" id="last_name" name="last_name" value="{{ old('last_name', $person?->last_name) }}" />
                            </div>
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="address_1" name="address_1" value="{{ old('address_1', $person?->address_1) }}" />
                            </div>
                            <div class="col-md-6">
                                <label class="form-label" for="address_2">Address 2</label>
                                <input type="text" class="form-control" id="address_2" name="address_2" value="{{ old('address_2', $person?->address_2) }}" />
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" id="state" name="state" value="{{ old('state', $person?->state) }}" />
                            </div>
                            <div class="col-md-4">
                                <label class="form-label" for="zip">Zip</label>
                                <input type="text" class="form-control" id="zip" name="zip" value="{{ old('zip', $person?->zip) }}" />
                            </div>
                            <div class="col-md-6">
                                <label class="form-label" for="country">Country</label>
                                <input type="text" class="form-control" id="country" name="country" value="{{ old('country', $person?->country) }}" />
                            </div>
                            <div class="col-md-12">
                                <label class="form-label" for="comments">Comments</label>
                                <textarea class="form-control" id="comments" name="comments" rows="3">{{ old('comments', $person?->comments) }}</textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-12">
                                <div class="form-check form-switch mb-3">
                                    <input class="form-check-input" type="checkbox" role="switch" id="override_default_tax" name="override_default_tax" value="1" @checked(old('override_default_tax', $supplier?->override_default_tax))>
                                    <label class="form-check-label" for="override_default_tax">Override Default Tax</label>
                                </div>
                            </div>
                            
                            <div id="tax_overrides_container" class="{{ old('override_default_tax', $supplier?->override_default_tax) ? '' : 'd-none' }}">
                                    ...
                                    <ul class="list-group mb-4">
                                        @foreach($files as $file)
                                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                                ...
                                                <div class="btn-group">
                                                    <a href="{{ route('suppliers.files.download', $file->file_id) }}" class="btn btn-sm btn-outline-primary" target="_blank"><i class="bi bi-download"></i></a>
                                                    <button type="button" class="btn btn-sm btn-outline-danger delete-file-btn" data-id="{{ $file->file_id }}"><i class="bi bi-trash"></i></button>
                                                </div>
                                            </li>
                                        @endforeach
                                    </ul>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" step="0.01" class="form-control" id="balance" name="balance" value="{{ old('balance', $supplier?->balance ?? 0) }}" />
                                </div>
                            </div>
                            <div class="col-md-12">
                                <label class="form-label" for="internal_notes">Internal Notes</label>
                                <textarea class="form-control" id="internal_notes" name="internal_notes" rows="4">{{ old('internal_notes', $supplier?->internal_notes) }}</textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row g-3">
                            @for($i = 1; $i <= 10; $i++)
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control" id="custom_field_{{ $i }}_value" name="custom_field_{{ $i }}_value" value="{{ old("custom_field_{$i}_value", $supplier?->{"custom_field_{$i}_value"}) }}">
                                </div>
                            @endfor
                        </div>
                    </div>

@php
    $defaultVisible = ['count_date', 'status', 'employee', 'items_counted', 'difference', 'cost_price_difference'];
@endphp
                ...
                <tr>
                    @foreach($headers as $label => $key)
                        ...
                        <td class="col-{{ $key }} {{ !in_array($key, $defaultVisible) ? 'col-hidden' : '' }}">
                            @if($key === 'status')
                                <span class="badge-status {{ strtolower($val) }}">{{ $val }}</span>
                            @elseif(in_array($key, ['difference', 'cost_price_difference']) && is_numeric($val))
...
document.addEventListener('DOMContentLoaded', function() {
    var STORAGE_KEY = 'summary_count_cols';

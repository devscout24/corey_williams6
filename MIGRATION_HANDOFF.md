...
- **Purchases (inventory receiving / returns):** Renamed user-facing module to **Purchases** (sidebar, register, dashboard links). Added canonical routes `GET /purchases` and `GET /purchases/create` (`purchases.index`, `purchases.create`); legacy `/receivings` URLs kept. Index rebuilt from POS HTML `pages/purchases.html`: toolbar + shared search (ID / supplier / date / status), **two tables** — purchases/receiving (all `phppos_receivings` rows where `mode <> 'return'`) and **returns** (`mode = 'return'`). `ReceivingController` cart POST redirects now return to `purchases.create` (fix: add-to-cart no longer bounced to the list). Seeder submodule label `receiving` → `Purchases` for new seeds.
